  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        ...
        options = {
          ...
          awslogs-stream-prefix = local.ureport-celery.namespace.worker
        }
      }
...
resource "aws_iam_role" "ureport_celery_worker_execution_role" {
  name               = "ECSTaskExecutionRole-${local.ureport-celery.namespace.worker}"
  assume_role_policy = data.aws_iam_policy_document.ecs_task_execution_policy.json
}

resource "aws_iam_role_policy_attachment" "ureport_celery_worker_execution_role_policy" {
